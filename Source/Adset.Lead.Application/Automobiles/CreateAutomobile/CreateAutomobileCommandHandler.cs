...
internal sealed class CreateAutomobileCommandHandler : ICommandHandler<CreateAutomobileCommand, Guid> 
{
    ...
    public async Task<Result<Guid>> Handle(CreateAutomobileCommand request, CancellationToken cancellationToken)
    {
        ...
        // Combina os opcionais em flags usando bitwise operations
        var optionalFeatures = CombineOptionalFeatures(request.OptionalFeatures);
    ...
    /// <summary>
    /// Combina as características opcionais em um único valor usando operações bitwise.
    /// </summary>
    private static OptionalFeatures CombineOptionalFeatures(IReadOnlyCollection<OptionalFeatures>? features)
    {
        if (features is null || !features.Any())
            return 0;

        var combinedFeatures = features.First();
